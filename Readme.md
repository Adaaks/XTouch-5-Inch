...
# Instructions
- Install visual studio code, then install platformio extension with espresiff32 libary and python, once that's done, you can build the firmware, which is on the bottom left small icon, then plug in your screen using a usb c data cable, then you can press upload whilst holding the BOOT button on the screen board, you can let go after it starts writing the firmware.
